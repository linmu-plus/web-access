...
它也能关掉**没开放调试端口的实例**（见下节），所以启动报「profile 已被占用」时，直接跑它就是正确的处置。

### 启动失败：先怀疑残留实例，别怀疑浏览器

`check-deps.mjs` / `launch-browser.mjs` 报 **「60 秒内专用实例未就绪」** 时，绝大多数情况**不是**浏览器或参数有问题，而是**上一轮拉起的实例还占着 profile**：

Chrome 的单例机制会把新的启动参数交给既有实例、然后自身退出。那个既有实例没开调试端口，于是 9222 永远不会监听、本脚本干等到超时。实测特征：失败期间 profile 目录**零写入**（Chrome 在动 profile 之前就退出了）。

**不要**据此去改启动参数或怀疑 Chrome 版本不支持调试端口——先执行：

```bash
node "<skill-base-dir>/scripts/close-browser.mjs"
```

现在脚本会在启动前**主动检测占用**，命中后 1 秒内直接报出原因并指向上述命令，不再让你干等 60 秒后猜错方向。
