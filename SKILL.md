...
专用实例**是**用户的登录态载体，日常任务中应该长驻，不要每轮任务都关：

- **长驻（默认）**：正常调研/抓取任务结束后，保持实例与 proxy 运行。冷启动一次 Chrome 要几十秒，反复拉起还会丢失登录态，并让用户反复面对权限确认弹窗。
- **必须关闭**：**验证、排查、临时试验、冒烟测试**这类为了"确认某个功能能不能用"而拉起的实例，用完立刻关掉。不能留在后台——它占着 9222 端口、挂着内存，用户下次会看到一个来路不明的浏览器窗口。自己拉起的临时实例，自己负责收尾。

关闭命令：

```bash
node "<skill-base-dir>/scripts/close-browser.mjs"
```

关闭走 CDP `Browser.close`（优雅退出，profile 正常落盘），失败才回退按 pid 终止。**只关专用隔离实例**，不会碰用户日常浏览器；实例本就没在运行时正常退出，可放心重复执行。
